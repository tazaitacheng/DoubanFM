...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary
{
    if (self = [super init]) {
        ...
        NSDictionary* tempPlayReordDic = [tempUserInfoDic valueForKey:@"play_record"];
        self.banned = [NSString stringWithFormat:@"%@",[tempPlayReordDic valueForKey:@"banned"]];
        self.liked = [NSString stringWithFormat:@"%@",[tempPlayReordDic valueForKey:@"liked"]];
        self.played = [NSString stringWithFormat:@"%@",[tempPlayReordDic valueForKey:@"played"]];
    }
    return self;
}
...
- (void)archiverUserInfo
{
    ...
    if ([data writeToFile:appdelegatePath atomically:YES]) {
        NSLog(@"UserInfo存储成功");
        NSLog(@"homePath:%@/%@",homePath,appdelegatePath);
    }
